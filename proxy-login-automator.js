...
//convert `--key value` to cfg[key]=value
    ...
    cfg.local_port = Number(cfg.local_port);
    cfg.remote_port = Number(cfg.remote_port);

    if (!cfg.local_port || !cfg.remote_host || !cfg.usr || !cfg.pwd)
        return console.error('Usage of parameters:\n'
            ...
            + '\t[--pac_url]\t\t' + 'URL of pac (proxy auto configuration script)\n'
    ...
    if (cfg.pac_url) {
        createPacServer(cfg.local_host, cfg.local_port, cfg.remote_host, cfg.remote_port, cfg.buf_proxy_basic_auth, cfg.pac_url);
    } else {
...
var proxyAddrMap = {};
function createPacServer(local_host, local_port, remote_host, remote_port, buf_proxy_basic_auth, pac_url) {
    var i = 0;
    net.createServer({allowHalfOpen: true}, function (socket) {
        var realCon = net.connect({port: remote_port, host: remote_host, allowHalfOpen: true});
        ...
        realCon.on('data', function (buf) {
            ...
            buf_ary = [];
            s = s.replace(/(['"])PROXY\s+([^'":\s]+)(:\d+)?\1/g, function (all, _q, _remote_host, _comma_remote_port) {
                var _remote_port = _comma_remote_port && Number(_comma_remote_port.slice(1)) || 80;
                var remoteAddr = _remote_host + ':' + _remote_port;
                var _local_port = proxyAddrMap[remoteAddr];
                if (!_local_port) {
                    _local_port = local_port + (++i);
                    proxyAddrMap[remoteAddr] = _local_port;
                    createPortForwarder(local_host, _local_port, _remote_host, _remote_port, buf_proxy_basic_auth);
                }
                return _q + 'PROXY ' + local_host + ':' + _local_port + _q;
            });
            s = s.replace(/Content-Length:.*$/i, '');
        ...
        realCon.end('GET ' + pac_url + ' HTTP/1.1\r\n'
        ...
        socket.on('data', dummy).on('error', dummy);
    }).on('error', function (err) {
        console.log('Failed to listen at ' + local_host + ':' + local_port + '\n' + err);
        process.exit(1);
    }).listen(local_port, local_host === '*' ? undefined : local_host, function () {
